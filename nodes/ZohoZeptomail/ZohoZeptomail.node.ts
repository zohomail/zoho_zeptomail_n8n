    ...
    methods = {
		loadOptions: {
			...
			async getListTemplate(this: ILoadOptionsFunctions) {
				const mailagent = this.getCurrentNodeParameter('mailagent', { extractValue: true }) as string;;
			...
			try {
				if (resource === 'message') {
					...
					if (operation === 'sendmail') {
						...
						responseData = await zohoZeptomailApiRequest.call(this, 'POST', `v1.0/email/template`,body ,{});
						responseData = responseData.data;
					}
				}
			}
