...
export function throwOnErrorStatus(
    ...
    if ((responseData as { error?: { details: Array<{ code: string; message: string }>; message: string; code: string; } }).error) {
        const error = (responseData as { error: { details: Array<{ code: string; message: string }>; message: string; code: string; } }).error;
...
export async function zohoZeptomailApiRequest(
	...
	const { oauthTokenData } = await this.getCredentials<ZohoZeptomailOAuth2ApiCredentials>('zohoZeptomailOAuth2Api');
...
export async function getPicklistMailagentOptions(
...
export async function getPicklistTemplateOptions(
	this: ILoadOptionsFunctions,
	targetField: string
) {
	const responseData = (await zohoZeptomailApiRequest.call(
